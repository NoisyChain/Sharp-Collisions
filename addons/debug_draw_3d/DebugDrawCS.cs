...
#region Draw Functions
	...
	public static void DrawArcLine(Vector3 position, Vector3 start, Vector3 end, float radius, Color? color = null, float duration = 0f){
		Vector3 a = position + start * radius;
		Vector3 b = position + end * radius;
		Vector3 ab = position + (start + end).Normalized() * radius;

		Vector3 aab = position + (start + (start + end)).Normalized() * radius;
		Vector3 abb = position + (end + (start + end)).Normalized() * radius;

		debug_draw_3d?.Call("draw_line", a, aab, color == null? empty_color : color, duration);
		debug_draw_3d?.Call("draw_line", aab, ab, color == null? empty_color : color, duration);
		debug_draw_3d?.Call("draw_line", ab, abb, color == null? empty_color : color, duration);
		debug_draw_3d?.Call("draw_line", abb, b, color == null? empty_color : color, duration);
	}
